...
for (i in seq(1,6)) {
  setwd(dir = annee[i])
  setwd(dir = "Novembre")
  # Lecture des fichiers des mois de novembre de 2013 à 2017
  ...
  select_list_LP_G <- lecture_list_LP_G[2]
  select_list_MBDN_G <- lecture_list_MBDN_G[2]
  select_list_SA_G <- lecture_list_SA_G[2]
  select_list_SL_G <- lecture_list_SL_G[2]
  select_list_SP_G <- lecture_list_SP_G[2]
  select_list_LP_D <- lecture_list_LP_D[2]
  select_list_MBDN_D <- lecture_list_MBDN_D[2]
  select_list_SA_D <- lecture_list_SA_D[2]
  select_list_SL_D <- lecture_list_SL_D[2]
  select_list_SP_D <- lecture_list_SP_D[2]
  # Conversion des listes en format numerique
  conv_num_LP_G <- as.numeric(unlist(select_list_LP_G))
  conv_num_MBDN_G <- as.numeric(unlist(select_list_MBDN_G))
  conv_num_SA_G <- as.numeric(unlist(select_list_SA_G))
  conv_num_SL_G <- as.numeric(unlist(select_list_SL_G))
  conv_num_SP_G <- as.numeric(unlist(select_list_SP_G))
  conv_num_LP_D <- as.numeric(unlist(select_list_LP_D))
  conv_num_MBDN_D <- as.numeric(unlist(select_list_MBDN_D))
  conv_num_SA_D <- as.numeric(unlist(select_list_SA_D))
  conv_num_SL_D <- as.numeric(unlist(select_list_SL_D))
  conv_num_SP_D <- as.numeric(unlist(select_list_SP_D))
...
moy_LP_G_nov <- mean(x = Donnees_LP_G)
moy_MBDN_G_nov <- mean(x = Donnees_MBDN_G)
moy_SA_G_nov <- mean(x = Donnees_SA_G)
moy_SL_G_nov <- mean(x = Donnees_SL_G)
moy_SP_G_nov <- mean(x = Donnees_SP_G)
moy_LP_D_nov <- mean(x = Donnees_LP_G)
moy_MBDN_D_nov <- mean(x = Donnees_MBDN_G)
moy_SA_D_nov <- mean(x = Donnees_SA_G)
moy_SL_D_nov <- mean(x = Donnees_SL_G)
moy_SP_D_nov <- mean(x = Donnees_SP_G)

# Ecriture dans un fichier externe
setwd(dir = "~/Base de données/Moyenne/Novembre")
Moyenne_G <- c(moy_LP_G_nov, moy_MBDN_G_nov, moy_SA_G_nov, moy_SL_G_nov, moy_SP_G_nov)
Moyenne_D <- c(moy_LP_D_nov, moy_MBDN_D_nov, moy_SA_D_nov, moy_SL_D_nov, moy_SP_D_nov)
write.table(x = Moyenne, file = "Valeur moyenne Diffus par mois", row.names = )
